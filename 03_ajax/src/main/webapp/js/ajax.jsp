...
<body>
	...
	<!--
		Asynchronous JavaScript And XML
		 - 서버로부터 데이터를 가져와서 전체 페이지를 새로 고치지 않고
		   일부분만 로드할 수 있게 하는 기법.
		 ...
		 * 동기식 / 비동기식
		  ...
		  * 비동기식
		   ...
		   - 서버에서 결과를 반환해주기까지 오랜 시간이 걸리더라도 현재 페이지를 이용하는데 문제가 없다. 
	-->
		...
		function fetch1(){
			/* 
				fetch함수
				 ...
				 - Promise기반으로 설계되어 있으며 비동기 요청 성공시, 
				   실패시 처리를 위한 메서드를 제공한다.
				 
				 - fetch함수는 비동기 요청을 보내면 Promise객체를 반환하고, 
				   서버로부터 데이터를 응답받기 전까지 다른작업을 수행할 수 있다.
				   
				   Promise
				    - 비동기 작업의 처리결과를 나타내는 객체로 3가지 상태값을 가진다.
				    1) peding : 비동기 작업을 처리중인 상태(초기값)
				    2) resolved : 비동기 작업 처리 완료 상태(응답받은상태)
				    3) rejected : 비동기 작업 처리 실패 상태(응답 X)
			*/
			
			// fetch(url,[options])
			// - options : 요청에 대한 옵션(methods, headers, body, ...)
			...
			.catch(function(error){ // Promise객체의 결과가 reject되는 경우 실행할 함수
				console.log(error);
			})
