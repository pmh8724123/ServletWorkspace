...
public class ElServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 *  - 이 영역에 데이터를 담으면, 현재 요청을 받은 서블릿과, 포워딩받는 서블링/jsp에서 데이터를 사용할 수 있다.
		 *  - 값이 한 번 담기면, 재요청이 이루어지는 순간 이전 request는 소멸된다.
		 *  
		 * 4. PageContext(page scope)
		 *  - JSP에서만 사용가능한 영역으로, 하나의 JSP에서만 공유할 데이터를 선언하는 영역
		 * */
		
		// 1. requestScope에 데이터 저장
		request.setAttribute("classRoom", "G강의장");
		request.setAttribute("student", new Person("박무혁",27,"남자"));
